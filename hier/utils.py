...
class MultiTransforms:
    ...
    def __init__(self, mean_std, model, view=1):
        self.view = view
        self.model = model
        self.sz_crop = 224
        self.is_inception = mean_std[0][0] > 1
        self.is_vit = ('vit' in model) or ('deit' in model)
        self.scale = (0.2, 1.0) if self.is_vit else (0.08, 1.0)
        self.transform = transforms.Compose([
            RGBToBGR() if self.is_inception else Identity(),
            transforms.RandomResizedCrop(self.sz_crop, scale=self.scale, interpolation=Image.BICUBIC),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            ScaleIntensities([0, 1], [0, 255]) if self.is_inception else Identity(),
...
def get_params_groups(model, sup_loss=None, cluster_loss=None, lr=1e-5, fc_lr_scale=1, weight_decay=0.01):
    ...
    return [
        ...
        {"name": "lcas", "params": lcas, "lr_scale": 5e1, "weight_decay": weight_decay},
